...
The first Windows acceptance run verified a clean `npm ci`, all 18 current tests, and a successful patched `npm run build` for both extension and broker. The unpacked extension was then loaded in current Edge and the **Tool Shim · restart** overlay rendered successfully on `chatgpt.com`, reporting `watching` state.

The first live conversation traffic then verified that the current assistant-message selector detects real model output and that `hello`, `clock`, and `browser.tabs.list` execute end-to-end. With Auto Submit disabled, each result was inserted into the ChatGPT composer and could be manually submitted, verifying insert-only delivery. The large `browser.tabs.list` response also demonstrated that ordinary inline delivery remains functional for the current result size. Duplicate/streaming behavior is still intentionally left unchecked until stressed rather than inferred from these successful calls.
